...
export default function Home({ navigation }: HomeProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [musicShelf, ctoken] = await musicShelfProvider.fetch(ctx, "Home", continuation.token);
        ...
        <FlatList
          ...
          renderItem={
            ({ item }) => {
              ...
              return (
                <View className="mt-3" key={"musicShelf-" + item.title.toString()} >
                  <Text
                    className="text-neutral-900 dark:text-sky-50 text-lg font-semibold">{item.title}</Text>
                  <View className="mt-2" >
                    <FlatList horizontal
                      data={item.cards}
                      renderItem={
