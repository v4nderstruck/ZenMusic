...
export default function SongCard({item}: any) {
  ...
  if (displayType == 'full') {
      ...
      <View className="w-64 h-44 flex flex-col overflow-hidden">
        <View className="h-[75%] w-full flex flex-col justify-end items-end">
          {thumbnailUrl ? (
            ...
            <Image
              source={require('../mock/rick_astley_mock.jpeg')}
              className="absolute w-full h-full rounded-sm"
            />
          )}
        </View>
    ...
    return (
      <View className="w-44 h-44 flex flex-col overflow-hidden">
        <View className="h-[75%] w-full flex flex-col justify-end items-end">
          {thumbnailUrl ? (
            <FastImage
              source={{
        ...
        <View className="mt-2">
          <Text className="text-black dark:text-neutral-200 font-semibold">
            {compactString(title, 18)}
          </Text>
          <View className="flex flex-row justify-between w-full">
            <Text className="text-black dark:text-neutral-200 font-thin">
              {compactString(subtitle, 38)}
            </Text>
          </View>
        </View>
      </View>
    ...
    return (
      <View className="w-44 h-16 gap-1 flex flex-row overflow-hidden justify-center content-center items-center ">
